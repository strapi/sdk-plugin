...
import { findPackageJsonPath } from '../find-package-json';
...
/**
 * @description Load the package.json starting from the current working directory
...
export const loadPkg = async ({
  ...
  const pkgPath = await findPackageJsonPath(cwd);
 ...
 */
export const validatePkg = async ({ pkg }: { pkg: object }): Promise<PackageJson> => {
  try {
